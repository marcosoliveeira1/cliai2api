...
- **Interfaces**:
  ...
  - Headers Chat Completions mantêm os aliases de afinidade e o cache key em header. Responses replica a identidade observada (incluindo `x-opencode-org-id` e User-Agent quando recebidos); `prompt_cache_key`, `store:false`, `include:["reasoning.encrypted_content"]` e `tool_choice:"auto"` seguem no corpo Responses.
