...
| Criterion | Spec-defined outcome | `file:line` + assertion | Result |
| --------- | -------------------- | ----------------------- | ------ |
| WHEN `opencode/<responses-id>` + `stream:true` THEN request converts to Responses and returns OpenAI chunks (`delta.content`, `finish_reason`) + `data: [DONE]` | 1 call to `/v1/responses`, converted input, translated chunks, DONE-terminated | `zen_responses_test.go` — Responses body uses bare model + streamed `input`; Muse free test checks `/v1/responses` and `input` with no Chat Completions `messages`; output contains text chunks, finish and `[DONE]` | ✅ PASS |
